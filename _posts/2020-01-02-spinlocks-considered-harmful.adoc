...
An address of a lock can be used as a token to identify a specif queue (this is a http://man7.org/linux/man-pages/man2/futex.2.html[futex] API).
...
It was waiting for entropy for some reason, an the keypress provided it.
====
...
Take a second to appreciate the absence of `unsafe` blocks and cross-core communication in the above example! At worst, `init` will be called `number of cores` times.
