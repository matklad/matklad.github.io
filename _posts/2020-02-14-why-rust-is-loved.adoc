...
_First_, it is a relatively yong language, so it can have many "`obviously good`" things.
...
That is, the single crate can be known under different names in different dependant crates or, vice verse, two different crates might be known under equal names in different parts of the crate graph!
...
The canonical comparison function returns an `enum Ord { Less, Equal, Greater }`, you don't need to override all six comparison operators.
...
Discussion at https://www.reddit.com/r/rust/[/r/rust]
