...
Module["onRuntimeInitialized"] = () => {
  ...
  const djot_to_ast_json = Module.cwrap("djot_to_ast_json", "string", [
    "number",
    "string",
    "boolean",
  ]);
  ...
  const djot_to_ast_pretty = Module.cwrap("djot_to_ast_pretty", "string", [
    "number",
    "string",
    "boolean",
  ]);
  ...
  const djot_to_matches_json = Module.cwrap("djot_to_matches_json", "string", [
    "number",
    "string",
  ]);
  ...
  const djot_to_matches_pretty = Module.cwrap(
    "djot_to_matches_pretty",
    "string",
    ["number", "string"],
  );
  ...
  const djot_to_html = Module.cwrap("djot_to_html", "string", [
    "number",
    "string",
    "boolean",
  ]);
...
function convert() {
  ...
  if (mode == "astjson") {
    document.getElementById("result").innerText = JSON.stringify(
      JSON.parse(djot.to_ast_json(text, sourcepos)),
      null,
      2,
    );
  } else if (mode == "ast") {
    document.getElementById("result").innerText = djot.to_ast_pretty(
      text,
      sourcepos,
    );
  } else if (mode == "matches") {
    document.getElementById("result").innerText = djot.to_matches_pretty(text);
  } else if (mode == "matchesjson") {
    document.getElementById("result").innerText = JSON.stringify(
      JSON.parse(djot.to_matches_json(text)),
      null,
      2,
    );
  } else if (mode == "html") {
